...
const Header = () => {
    ...
    return (
        <header className="w-full text-sm body-font fixed sm:sticky top-0 left-0 z-20">

            <div className="text-2xl absolute top-2 left-2 z-30 bg-gray-500 p-2 rounded-full cursor-pointer sm:hidden" onClick={toggleTopNav}>
                <IoMenu className="text-white rounded-full border-transparent focus:border-slate-800" />
            </div>


            <nav className={`w-5/6 h-screen sm:h-fit sm:w-full bg-[rgba(255,255,255,0.6)] text-gray-700 flex flex-col sm:flex-row absolute sm:static ${showTopNav ? "left-0" : "-left-full"} sm:left-0 p-3 gap-5 sm:gap-10 items-center ustify-center backdrop-blur-sm shadow-md`}>
                ...
                <Link href="/" className="flex text-center items-center text-gray-900 border-2 outline-none border-transparent focus:border-slate-500 font-bold mb-2 sm:mb-0 rounded-full" onClick={(e)=>setShowTopNav(false)}>
                    <Image src="/images/logo.png" alt="logo" width={50} height={50} className="w-11 h-11 object-contain rounded-full" />
                </Link>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-center gap-5 text-center">
                    {
                        TOP_NAV_LINKS.map(({ id, title, link }) => (
                            <NavLink key={id} navRef={link} navLinkClass="capitalize cursor-pointer font-semibold outline-none border-b-2 border-transparent focus:border-slate-500 hover:text-black px-2 py-1" activeNavClass="border-b-2" navLinkClick={()=>setShowTopNav(false)}>
                                {title}
                            </NavLink>
                        ))
                    }
                ...
                <div className="mx-auto sm:ml-auto sm:mr-0">
                    {
                        !user.loggedIn && <div className="w-full flex flex-row justify-center items-center gap-4 font-semibold">
                            ...
                            <Link href="/login" className="flex flex-row justify-center items-center gap-2 hover:text-black outline-none border-transparent border-2 focus:border-slate-500 px-2 py-1">
