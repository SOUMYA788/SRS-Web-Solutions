        ...
        <>
...

            {
                user.loggedIn && <div className="relative">
                    {
                        user?.value?.userProfileColor && <div tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter") userIconHoverFunc() }} className={`w-11 h-11 rounded-full ${user?.value?.userProfileColor} border border-black cursor-pointer`} onClick={userIconHoverFunc}>
                            {
                                user?.value?.userProfilePicture && <Image src={`${user?.value?.userProfilePicture}`} alt="user" width={50} height={50} className="w-full h-full object-contain" />
                            }
                        </div>
                    }
                    ...
                    <div className={`absolute top-16 -left-24 p-3 bg-gray-100 rounded-md ${iconHover ? "block" : "hidden"} shadow-md border-2 select-none`}>
                        ...
                        {user?.value?.userName && <h2 className='text-center text-gray-600 mb-1'> {user
                            ?.value?.userName} </h2>}

                        {user?.value?.userEmail && <p className='text-center text-gray-400 text-xs mb-3'> {user?.value?.userEmail} </p>}

                        <Link href={`/dashbord/${user?.value?._id}`} className='text-gray-600 text-xs p-1 my-1 cursor-pointer capitalize tracking-wider' onClick={() => setIconHover(false)}>dashbord</Link>
