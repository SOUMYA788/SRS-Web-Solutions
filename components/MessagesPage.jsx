...
const MessagesPage = ({ messages }) => {
    ...
    const deleteSelectedMessages = async (e) => {
        e.preventDefault();
        if (selectMessages.length > 0) {
            console.log(selectMessages, "from MessagesPage component");
            const { success, message } = await deleteUserMessages([...selectMessages]);
            success ? showSuccessToast(message) : showErrorToast(message)
        } else {
            showErrorToast("No message select to delete")
        }
